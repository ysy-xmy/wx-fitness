          ...
          <div style="width: 100vw; padding: 0 20px">
            ...
            >
              ...
              <van-stepper
                ...
                min="0"
                step="1"
                @change="(e) => changeNum(item, e.detail)"
              />
            </div>
          </div>
        </div>
...
const changeNum = (item, val) => {
  chooseList.value.find((it) => it == item).num = val;
};
const subitClass = () => {
  //发布课程
  let temp = chooseList.value;
  console.log(temp);
  temp = chooseList.value.filter((item) => item.num != 0);
