...
//获取我的课程
export const getMycourese = (Size: number, Page: number) => {
  let data = {
    Size: String(Size),
    Page: String(Page),
  };
  // return request.get("/api/course/owner-courses", { params: data });
  return new Promise((res, rej) => {
    uni
      .request({
        url: "http://127.0.0.1:4523/m1/4822762-4477606-default/api/course/owner-courses",
        param: data,
        header: "Bear " + uni.getStorageSync("token"),
      })
      .then((r) => {
